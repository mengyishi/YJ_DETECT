...
def pred_img(img0):
    ...
    pred = non_max_suppression(pred, CONF_THRES, IOU_THRES, classes=list(range(21)), max_det=MAX_DET)
...
def press_key_control(detections_info):
    for detection in detections_info:
        ...
        if len(class_name) == 1 and class_name.isalpha():
            ...
            time.sleep(0.03)  # 模拟按键按住 30 毫秒
            pydirectinput.keyUp(class_name)
            #time.sleep(0.02)  # 增加延迟，避免过快输入
    time.sleep(0.01)  # 增加延迟，避免过快输入
#
